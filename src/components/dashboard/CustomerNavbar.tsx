...
export function CustomerNavbar() {
  ...
  async function handleLogout() {
    ...
    try {
      ...
      // Clear cookie on client side as well for immediate effect across paths
      document.cookie = "session=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT; max-age=0";
      // Perform full page redirect to completely purge all memory state and react cache
      window.location.replace("/login");
    }
  }

  function handleSelectMeal(meal: any) {
    setSearchOpen(false);
    const isDashboard = typeof window !== "undefined" && (window.location.pathname === "/dashboard" || window.location.pathname.startsWith("/dashboard"));

    let fallbackImg = meal.imageUrl;
    if (!fallbackImg || fallbackImg === "") {
      if (meal.name?.toLowerCase().includes("biryani")) fallbackImg = "/biryani_handi_slider.jpg";
      else if (meal.name?.toLowerCase().includes("burger")) fallbackImg = "/truffle_burger.jpg";
      else if (meal.name?.toLowerCase().includes("pizza")) fallbackImg = "/margherita_pizza.jpg";
      else if (meal.isVeg) fallbackImg = "/paneer_bowl_new.png";
    ...
    const fullMealData = {
      id: meal.id,
      name: meal.name,
      category: meal.categoryName || (meal.isVeg ? "Veg Delights" : "Artisan Non-Veg"),
      tag: meal.isVeg ? "PURE VEG" : "NON-VEG",
      tagType: meal.isVeg ? "PURE VEG" : "NON-VEG",
      rating: meal.rating ? Number(meal.rating) : 4.9,
      specs: meal.mealType ? `${meal.mealType} • Fresh Prep` : "Kitchen Special • Slow Dum",
      calories: meal.calories ? `${meal.calories} kcal` : "580 kcal",
      protein: meal.protein || "30g protein",
      description: meal.description || "Crafted fresh per order in authentic sealed terracotta handis & wood ovens.",
    ...
    if (isDashboard) {
      window.dispatchEvent(
        new CustomEvent("qbowl-select-meal", {
          detail: {
            mealId: meal.id,
            mealData: fullMealData,
          },
        })
      );
    } else {
      try {
        sessionStorage.setItem("qbowl_selected_meal", JSON.stringify(fullMealData));
      } catch (e) {
        console.error("Storage error:", e);
      }
      router.push(`/dashboard#meal-${meal.id}`);
    }
  }

  function handleSelectOrder(ord: { id: string; status: string }) {
    setSearchOpen(false);
    const isDashboard = typeof window !== "undefined" && (window.location.pathname === "/dashboard" || window.location.pathname.startsWith("/dashboard"));
    const isCompleted = ord.status === "DELIVERED" || ord.status === "CANCELLED";
    const targetHash = isCompleted ? "history" : "active-order";
  ...
  return (
    ...
    >
      <div className="w-full px-4 sm:px-8 lg:px-12 flex items-center justify-between gap-4 sm:gap-8">
        ...
        <div className="relative flex-1 max-w-xl lg:max-w-2xl mx-auto hidden sm:block">
          <div className="flex items-center gap-3 rounded-full border-2 border-black bg-white px-5 py-2.5 shadow-[2px_2px_0px_#000000] focus-within:shadow-[4px_4px_0px_#000000] focus-within:border-black transition-all">
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  if (searchResults?.meals && searchResults.meals.length > 0) {
                    handleSelectMeal(searchResults.meals[0]);
                  } else if (searchResults?.orders && searchResults.orders.length > 0) {
                    ...
                    if (isDashboard) {
                      ...
                      router.push("/dashboard#menu-section");
                    }
                  }
                }
              }}
                    ...
                    {searchResults.meals.map((meal) => (
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          <span
                            className={`h-2.5 w-2.5 rounded-full ${meal.isVeg ? "bg-emerald-500" : "bg-red-500"
                              }`}
        ...
        {/* 3 & 4. Right Actions: Search (Mobile) + Bell + Cart + Profile (Desktop only) */}
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">

          {/* Mobile Search Icon (Naked small icon on phone, hidden on desktop) */}
          <div className="sm:hidden relative">
            <button
              ...
              className="p-1.5 text-black hover:text-[#E5A00D] transition-colors"
              aria-label="Search bowls"
              title="Search"
            >
          ...
          {/* 3. Bell Icon (Notifications) - Naked small on phone, bordered circle on desktop */}
          <div className="relative">
            <button
              ...
              className={`relative p-1.5 sm:p-2.5 rounded-full sm:border-2 sm:border-black transition-all sm:bg-white sm:shadow-[2px_2px_0px_#000000] sm:hover:shadow-[3px_3px_0px_#000000] sm:hover:bg-[#FFF8EE] active:scale-95 ${notifOpen ? "text-[#E5A00D] sm:text-black sm:bg-[#E5A00D]" : "text-black"
                }`}
              aria-label="Notifications"
              title="Notifications"
            >
              <Bell size={18} className="stroke-[2.2] sm:stroke-2" />
              {unreadCount > 0 && (
                <span className="absolute -top-0.5 sm:-top-1 -right-0.5 sm:-right-1 h-3.5 min-w-[14px] sm:h-5 sm:min-w-[20px] px-0.5 sm:px-1 bg-[#E5A00D] text-black font-black text-[8px] sm:text-[10px] rounded-full flex items-center justify-center border border-black sm:border-2 sm:border-black animate-pulse shadow-sm">
                  {unreadCount}
                </span>
              )}
            </button>
          ...
          {/* 3.5 Cart Icon Button - Naked small on phone, bordered circle on desktop */}
          <div className="relative">
            <button
              onClick={() => {
              ...
              className={`relative p-1.5 sm:p-2.5 rounded-full sm:border-2 sm:border-black transition-all sm:bg-white sm:shadow-[2px_2px_0px_#000000] sm:hover:shadow-[3px_3px_0px_#000000] sm:hover:bg-[#FFF8EE] active:scale-95 ${cartOpen ? "text-[#E5A00D] sm:text-black sm:bg-[#E5A00D]" : "text-black"
                }`}
              aria-label="View Shopping Cart"
              title="Your Food Bowl Cart"
            >
              <ShoppingCart size={18} className="stroke-[2.2] sm:stroke-2" />
              {cartCount > 0 && (
                <span className="absolute -top-0.5 sm:-top-1 -right-0.5 sm:-right-1 h-3.5 min-w-[14px] sm:h-5 sm:min-w-[20px] px-0.5 sm:px-1 bg-black text-[#E5A00D] font-black text-[8px] sm:text-[10px] rounded-full flex items-center justify-center border border-[#E5A00D] sm:border-2 sm:border-[#E5A00D] shadow-sm">
                  {cartCount}
                </span>
              )}
            </button>
            ...
            {cartOpen && (
              <div className="absolute right-0 mt-3 w-[calc(100vw-24px)] max-w-sm sm:w-88 bg-white border-2 border-black rounded-2xl shadow-[6px_6px_0px_#000000] p-4 z-50 animate-in fade-in zoom-in-95 duration-150 space-y-3">
                ...
                <button
                  ...
                  className={`w-full py-3 rounded-xl border-2 font-outfit font-black text-xs uppercase tracking-wider transition-all flex items-center justify-center gap-2 ${kitchenClosed
          ...
          {/* 4. Profile Icon (Visible only on Desktop / Big screens, hidden on mobile) */}
              ...
              <div className="absolute right-0 mt-3 w-64 bg-white border-2 border-black rounded-2xl shadow-[6px_6px_0px_#000000] p-3 z-50 animate-in fade-in zoom-in-95 duration-150 space-y-2">
                ...
                <div className="border-t-2 border-black/10 pt-1">
                  <button
                    ...
                    className="w-full flex items-center justify-between px-3 py-2 text-xs font-bold text-red-600 hover:bg-red-50 rounded-xl transition-colors disabled:opacity-60 cursor-pointer"
                  >
                    <div className="flex items-center gap-2">
                      {isLoggingOut ? (
      ...
      {searchOpen && (
        <div className="sm:hidden px-4 pt-2 pb-1 animate-in fade-in slide-in-from-top-2 duration-150 relative">
          <div className="flex items-center gap-2.5 rounded-2xl border-2 border-black bg-white px-3.5 py-2 shadow-[3px_3px_0px_#000000]">
            {isSearching ? (
            ...
            <input
              ...
              placeholder="Search bowls, meals, orders..."
              className="w-full bg-transparent text-xs outline-none placeholder:text-zinc-400 text-black font-semibold"
            />
            <button
              onClick={() => {
                setSearchQuery("");
          ...
          {searchResults && (
            <div className="mt-2 bg-white border-2 border-black rounded-2xl shadow-[4px_4px_0px_#000000] p-3 max-h-72 overflow-y-auto space-y-2.5">
              {/* Food & Bowls */}
              {searchResults.meals.length > 0 && (
                <div>
                  <div className="flex items-center gap-1.5 px-1 mb-1 text-[11px] font-black uppercase tracking-wider text-[#E5A00D]">
                  ...
                  <div className="space-y-1">
                    {searchResults.meals.map((meal) => (
                      <div
                        key={meal.id}
                        className="flex items-center justify-between p-2 hover:bg-[#FFF8EE] rounded-xl border border-transparent hover:border-black/10 transition-colors cursor-pointer"
                        onClick={() => handleSelectMeal(meal)}
                      >
                        <div className="flex items-center gap-2">
                          <span
                            className={`h-2 w-2 rounded-full ${meal.isVeg ? "bg-emerald-500" : "bg-red-500"
                              }`}
                          />
                          <div>
                            <p className="text-xs font-bold text-black">{meal.name}</p>
                            {meal.description && (
                              <p className="text-[10px] text-zinc-500 line-clamp-1">{meal.description}</p>
                            )}
                          </div>
                        </div>
                        <span className="text-xs font-black text-black bg-[#FFF8EE] px-1.5 py-0.5 rounded border border-black/10">
                          ₹{meal.price}
                        </span>
                      </div>
                    ))}
              ...
              {searchResults.orders.length > 0 && (
                <div>
                  <div className="flex items-center gap-1.5 px-1 mb-1 text-[11px] font-black uppercase tracking-wider text-black/60">
                    <Package size={13} />
                    <span>Your Orders ({searchResults.orders.length})</span>
                  </div>
                  <div className="space-y-1">
                    {searchResults.orders.map((ord) => (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-[11px] font-mono font-bold text-black">{formatOrderId(ord.id)}</p>
                            <p className="text-[10px] text-zinc-500 capitalize">{ord.status.toLowerCase().replace(/_/g, " ")}</p>
                          </div>
                        </div>
                        <span className="text-xs font-bold text-black">₹{ord.total}</span>
                      </div>
