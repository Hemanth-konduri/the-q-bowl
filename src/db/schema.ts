...
export const kitchenSettings = pgTable("kitchen_settings", {
  ...
  kitchenStatus: text("kitchen_status").default("OPEN").notNull(),
  openingTime: text("opening_time").default("07:00 AM").notNull(),
  closingTime: text("closing_time").default("10:30 PM").notNull(),
  isOrderingPaused: boolean("is_ordering_paused").default(false).notNull(),
  maxOrdersPerDay: integer("max_orders_per_day").default(250).notNull(),
  currentOrdersToday: integer("current_orders_today").default(0).notNull(),
  isSameDayOrderingEnabled: boolean("is_same_day_ordering_enabled").default(true).notNull(),
  estimatedPrepTime: text("estimated_prep_time").default("25 - 35 mins").notNull(),
  isHolidayClosure: boolean("is_holiday_closure").default(false).notNull(),
  holidayReason: text("holiday_reason").default("Special Holiday"),
  holidayReopeningDate: text("holiday_reopening_date"),
  isNoticeBannerActive: boolean("is_notice_banner_active").default(true).notNull(),
  noticeBannerText: text("notice_banner_text").default("Kitchen is open and serving fresh homemade bowls! Pre-orders welcome."),
  noticeBannerType: text("notice_banner_type").default("INFO").notNull(),
  subscriberExemptionNote: text("subscriber_exemption_note").default("Active subscribers continue receiving daily scheduled meals on time without interruption."),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
